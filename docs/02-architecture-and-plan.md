...
| 花火のアニメーション | 上昇 → 開花 → フェードの演出。連続再生／停止／1発だけ／打上間隔の操作。Entity ＋ `CallbackProperty` で実装し、`viewer.clock` は動かさない（昼夜判定に使っているため） |
...
- **花火のアニメーションの写実性向上**（`Cesium.ParticleSystem` による粒子表現）。
  デモ版では Entity ＋ `CallbackProperty` の軽量な演出のみ実装済み。
  粒子表現は描画負荷が上がるため、設定で ON/OFF できるようにすること。
  写実性が上がるほど誤認のリスクも上がる点に注意（`04-risks.md` R14）。
- 打上プログラム（時刻・号数）に沿った再生。**公式のプログラムが公表される場合のみ**。
  公表されない場合に仮のプログラムを使うと、それ自体が誤認のもとになる。
