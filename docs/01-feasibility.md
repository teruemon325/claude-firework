...
- 🔶 今回の調査で存在を確認できたのは **2022 年度（令和 4 年度）整備**版（標準製品仕様書 v2 系準拠）です。
...
| 地物型 | `type_en` | 本アプリでの用途 | 収録有無 |
|---|---|---|---|
| 建築物モデル | `bldg` | 花火を遮る建物の概略把握（最重要） | ⚠️ 未確認 |
| 地形モデル | `dem` | 地盤標高、目線高さ、高低差の算出 | ⚠️ 未確認 |
| 道路モデル | `tran` | 会場周辺の位置関係の把握、都南大橋の表現 | ⚠️ 未確認 |
| 土地利用モデル | `luse` | 周辺状況の参考表示（任意） | ⚠️ 未確認 |
...
- 確認方法（いずれか）：
  - PLATEAU VIEW（`https://www.plateau-view.mlit.go.jp/`）で盛岡市の各モデルを表示し、
